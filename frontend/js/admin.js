...
export async function cargarDatosAdminResumen() {
    try {
        const [ventasData, usuariosData, stockData, topClientesData] = await Promise.all([
        ...
        if(topClientesData.length === 0) {
            ...
            clientsContainer.innerHTML = `
                <table class="admin-table">
                    ...
                    <tbody>
                        ${topClientesData.slice(0, 5).map(c => `
                            <tr>
                                <td>${c.nombre}</td>
    ...
    try {
        ...
        html += usuarios.map(u => `
            <tr>
                ...
                <td>
                     <button class="btn-sm btn-secondary" onclick="window.abrirModalUsuario(${u.id_cliente}, '${u.nombre}', '${u.email}', '${u.telefono || ''}')">Editar</button>
                     <button class="btn-sm btn-danger" onclick="window.eliminarUsuario(${u.id_cliente})">Eliminar</button>
                </td>
            </tr>
        `).join('');
        html += '</tbody></table>';
        ...
        div.innerHTML = provs.map(p => `
            <div class="card-proveedor" style="border: 1px solid #ddd; padding: 15px; border-radius: 8px; background: white;">
                ...
                <div style="margin-top: 10px;">
                    <button class="btn-sm btn-secondary" onclick="window.showModalNuevoProveedor(${p.id_proveedor}, '${p.nombre}', '${p.telefono || ''}')">Editar</button>
                    <button class="btn-sm btn-danger" onclick="window.eliminarProveedor(${p.id_proveedor})">Eliminar</button>
                </div>
            </div>
        `).join('');
    } catch (error) {
...
export async function cargarHistorialVentasGlobal() {
    ...
    try {
        ...
        ventas.forEach(v => {
            const clienteNombre = usersCache[v.id_cliente] || `Cliente #${v.id_cliente}`;
            html += `<tr>
                <td>#${v.id_venta}</td>
                <td>${new Date(v.fecha).toLocaleDateString()}</td>
                <td>${v.id_cliente ? clienteNombre : 'Visitante'}</td>
                <td>$${v.monto_total.toFixed(2)}</td>
                <td><button class="btn-sm" onclick="window.abrirModalDetalleVenta(${v.id_venta})">Ver</button></td>
            </tr>`;
        });
        html += '</tbody></table>';
        list.innerHTML = html;
        ...
        fetchData(`/api/productos/${id}`).then(p => {
             ...
             if(p.detalles_subtipo) {
                 if(p.tipo_producto === 'ropa') {
                     ...
                     document.getElementById('calzado-talla').value = p.detalles_subtipo.talla || '';
             ...
             cargarProveedoresSelect().then(() => {
                 if(p.id_proveedor) document.getElementById('producto-proveedor').value = p.id_proveedor;
             });
        });
    } else {
...
async function handleProductoSubmit(e) {
    ...
    const basicData = {
        ...
        id_proveedor: parseInt(document.getElementById('producto-proveedor').value) || null,
...
// Open User Modal
// Open User Modal
export function abrirModalUsuario(id = null, nombre='', email='', telefono='') {
...
export async function abrirModalDetalleVenta(id) {
    ...
    // Reset contents
    document.getElementById('detalle-venta-items').innerHTML = '<tr><td colspan="4">Cargando...</td></tr>';
    ...
    try {
        ...
        // Header Info
        document.getElementById('detalle-venta-fecha').textContent = new Date(venta.fecha).toLocaleString();
        document.getElementById('detalle-venta-cliente').textContent = venta.nombre_cliente || 'Desconocido';
        
        // Items
        let htmlItems = '';
        if(venta.detalles && venta.detalles.length > 0) {
            htmlItems = venta.detalles.map(d => {
            ...
            htmlItems = '<tr><td colspan="4" style="text-align:center;">Sin detalles disponibles</td></tr>';
        }
        
        document.getElementById('detalle-venta-items').innerHTML = htmlItems;
        document.getElementById('detalle-venta-total-final').textContent = `$${venta.monto_total.toFixed(2)}`;
...
document.addEventListener('DOMContentLoaded', () => {
    // Modal Closers
    document.getElementById('cerrar-modal-admin-producto')?.addEventListener('click', () => hideModal(document.getElementById('modal-admin-producto')));
    document.getElementById('cerrar-modal-admin-proveedor')?.addEventListener('click', () => hideModal(document.getElementById('modal-admin-proveedor')));
    document.getElementById('cerrar-modal-admin-usuario')?.addEventListener('click', () => hideModal(document.getElementById('modal-admin-usuario')));
    document.getElementById('cerrar-modal-detalle-venta')?.addEventListener('click', () => hideModal(document.getElementById('modal-detalle-venta')));

    // Forms
    document.getElementById('form-nuevo-producto')?.addEventListener('submit', handleProductoSubmit);
    document.getElementById('form-nuevo-proveedor')?.addEventListener('submit', handleProveedorSubmit);
    document.getElementById('form-nuevo-cliente')?.addEventListener('submit', handleUsuarioSubmit);
});
